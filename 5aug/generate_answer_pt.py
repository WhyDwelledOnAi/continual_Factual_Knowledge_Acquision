...
def parse_args():
    ...
    parser.add_argument('--gpu_percent', type=float, default=0.9)
...
def calc_FTA():
    json_root = '/'.join(args.model_path.split('/')[:-1])
    json_name = '[Original]first_token_accuracy_history.json'
    if args.mode == 'continual':
        json_name = '[Continual]first_token_accuracy_history.json'
    with open(json_root+'/'+json_name, 'r', encoding='utf8') as f:
        fta_data = json.load(f)
    
    attributes = ['Birthdate', 'Birthplace', 'University', 'Major', 'Company']
    
    total = 0
    best_hFTA, best_sFTA = 0., 0.
    for example in fta_data.values():
        if total ==0:
            for attr in attributes:
                total += example[attr]['total']
        hFTA_avg, sFTA_avg = 0., 0.
        for attr in attributes:
            hFTA_avg += (example[attr]['hard_correct'] / total)
            sFTA_avg += (example[attr]['soft_correct'] / total)
        if best_hFTA < hFTA_avg:
            best_hFTA = hFTA_avg
            best_sFTA = sFTA_avg
    print("Best hFTA:", best_hFTA*100)
    print("Best sFTA:", best_sFTA*100)
