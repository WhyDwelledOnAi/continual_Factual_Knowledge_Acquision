...
def parse_args():
    ...
    parser.add_argument('--model_path', type=str, 
        default='outputs/cpt-naive-pythia/final_model')
...
if args.mode not in ['original', 'continual']:
    print("Wrong mode, change to continual")
    args.mode = 'continual'
...
def load_prompts2():
    ...
    if args.mode == 'original':
        with open(f'data_{args.model}/pt_test.pkl', 'rb') as f:
            all_tokens = pickle.load(f)
    else:
        with open(f'data_{args.model}/cpt_test.pkl', 'rb') as f:
            all_tokens = pickle.load(f)
...
def calc_EM():
    ...
    print("Accuracy:", answers['Match'].mean()*100)
