...
def parse_args():
    ...
    parser.add_argument('--wandb_key', type=str, 
        default='1e11cab943c0b40b8e674f9d50ab25d7bb576198')
    
    # data, tokenizer and model
    parser.add_argument('--data_path', type=str, default='data/')
    parser.add_argument('--init_model_path', type=str, 
        # default='/new_disk2/haoyu_wang/LLMs/pythia-160m',
...
class MyTrainer(Trainer):
    ...
    def set_gradient_basis(self):
        ...
        for name, module in module_dict.items():
            def make_hook(key):
                def hook(module, inputs, output):
                    # inputs是tuple，线性层输入形状通常为[bs, seq, hidden]或[bs*seq, hidden]
                    inp = inputs[0]
                    if inp is None:
                        return
                    # 统一展平到二维 [N, hidden]
        ...
        for layer_path, feats in tqdm(activation_buffers.items(), desc='Building GPM'):
            ...
            # 记录到参数映射：线性层权重命名为"{layer_path}.weight"
